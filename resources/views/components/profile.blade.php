<x-layout :doctitle="$doctitle">
    <div class="container py-md-5 container--narrow">
        <h2>
          ...
          {{-- checks only for logged in user --}}
          @auth
            <form class="ml-2 d-inline" action="/create-follow/{{$sharedData['username']}}" method="POST">
                @csrf
                @if (!($sharedData['currentlyFollowing'] > 0 || auth()->user()->username == $sharedData['username']))
            ...
            <form class="ml-2 d-inline" action="/remove-follow/{{$sharedData['username']}}" method="POST">
                @csrf
                @if (auth()->user()->username != $sharedData['username'] && $sharedData['currentlyFollowing'] > 0)
                    <button type="submit" class="btn btn-danger btn-sm">Stop Following <i class="fas fa-user-times"></i></button>
                @endif
            </form>
